...
export const MovieCardSmall = () => {
  ...
  const [expandedOverview, setExpandedOverview] = useState<{ [key: number]: boolean }>({});
  ...
  return (
    <div className="w-[100%] max-w-[1200px] ">
      <Swiper navigation modules={[Navigation]} className="w-[90%] bg-white text-white"
        >
        {movies.map((movie) => (
          <SwiperSlide key={movie.id} >
            <Link href={`/movie/${movie.id}`} passHref>
              ...
              <div className="flex items-center justify-between w-[100%] text-start my-[10px]">
                ...
                <h3 className="text-sm w-[200px] text-black  ml-[10px] md:text-l">Now Playing:</h3>
                <h3 className="text-l font-bold text-black truncate w-[90%] md:text-2xl">{movie.title}</h3>
                </div>
                <div className="flex items-center mt-2 mr-[3%] md:text-xl">
                  <FaStar className="text-yellow-500 text-xs md:text-xl" />
                  <p className="text-black text-xs mr-[3%] md:text-xl">{movie.vote_average}<span className="text-blue-400 mr-[3%]">/10</span></p>
                </div>
              </div>
              <p className="text-black text-sm w-[320px] text-start m-[1%] md:text-xl">
